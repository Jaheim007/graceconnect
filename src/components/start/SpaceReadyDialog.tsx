import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';
import { SiteLogo } from '@/components/ui/SiteLogo';
import { useAuth } from '@/contexts/AuthContext';
import { useI18n } from '@/i18n/I18nContext';
import { Loader2, Mail, Sparkles } from 'lucide-react';
import { toast } from 'sonner';

interface Props {
  open: boolean;
  onOpenChange: (v: boolean) => void;
  activityLabel?: string;
  spaceName?: string;
  returnTo?: string; // default: /start/finish
}

export function SpaceReadyDialog({ open, onOpenChange, activityLabel, spaceName, returnTo = '/start/finish' }: Props) {
  const { signInWithGoogle } = useAuth();
  const { locale } = useI18n();
  const navigate = useNavigate();
  const fr = locale === 'fr';
  const [googleLoading, setGoogleLoading] = useState(false);

  const handleGoogle = async () => {
    setGoogleLoading(true);
    const { error } = await signInWithGoogle(returnTo);
    if (error) {
      setGoogleLoading(false);
      toast.error(error.message);
    }
  };

  const handleEmail = () => {
    navigate(`/auth?mode=signup&returnTo=${encodeURIComponent(returnTo)}`);
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md p-0 overflow-hidden border-border/60">
        {/* Brand header */}
        <div className="bg-gradient-to-br from-primary/10 via-background to-background px-6 pt-8 pb-6 text-center">
          <div className="flex flex-col items-center gap-3">
            <div className="relative">
              <div className="absolute inset-0 blur-2xl bg-primary/40 rounded-full" />
              <div className="relative h-14 w-14 rounded-2xl bg-primary text-primary-foreground grid place-items-center shadow-lg shadow-primary/30">
                <Sparkles className="h-6 w-6" />
              </div>
            </div>
            <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              <SiteLogo size="xs" />
              <span>Siteviral</span>
            </div>
          </div>
          <DialogHeader className="mt-4 space-y-2">
            <DialogTitle className="text-2xl font-black tracking-tight text-center">
              {fr ? '🎉 Votre espace est prêt !' : '🎉 Your space is ready!'}
            </DialogTitle>
            <DialogDescription className="text-sm text-center">
              {spaceName ? (
                <span className="font-semibold text-foreground">{spaceName}</span>
              ) : null}
              {spaceName && activityLabel ? ' · ' : null}
              {activityLabel ? <span>{activityLabel}</span> : null}
              <span className="block mt-2 text-muted-foreground">
                {fr
                  ? 'Connectez-vous pour accéder à votre tableau de bord.'
                  : 'Sign in to access your dashboard.'}
              </span>
            </DialogDescription>
          </DialogHeader>
        </div>

        {/* Auth actions */}
        <div className="px-6 pb-6 space-y-3">
          <Button
            variant="outline"
            className="w-full h-12 gap-2.5 text-sm font-semibold"
            onClick={handleGoogle}
            disabled={googleLoading}
          >
            {googleLoading ? (
              <Loader2 className="h-5 w-5 animate-spin" />
            ) : (
              <svg className="h-5 w-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
            )}
            {googleLoading
              ? (fr ? 'Redirection…' : 'Redirecting…')
              : (fr ? 'Continuer avec Google' : 'Continue with Google')}
          </Button>

          <Button
            className="w-full h-12 gap-2.5 text-sm font-semibold"
            onClick={handleEmail}
          >
            <Mail className="h-5 w-5" />
            {fr ? 'Continuer avec votre email' : 'Continue with your email'}
          </Button>

          <p className="text-[11px] text-muted-foreground text-center pt-1">
            {fr
              ? 'Sans mot de passe. Un code vous sera envoyé.'
              : 'No password needed. A code will be sent to you.'}
          </p>
        </div>
      </DialogContent>
    </Dialog>
  );
}
