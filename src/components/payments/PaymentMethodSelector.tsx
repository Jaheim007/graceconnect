...
export function PaymentMethodSelector({ value, onChange, currency, className, paystackEnabled = true }: PaymentMethodSelectorProps) {
  ...
  const showMoneroo = true; // Moneroo enabled — routes through Paystack gateway
  ...
  const gridCols = methodCount >= 5 ? 'grid-cols-3' : methodCount >= 4 ? 'grid-cols-2' : methodCount === 3 ? 'grid-cols-3' : methodCount === 2 ? 'grid-cols-2' : 'grid-cols-1';
