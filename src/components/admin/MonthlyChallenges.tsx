...
export function MonthlyChallenges() {
  ...
  const challenges: Challenge[] = useMemo(() => [
    {
      id: 'sales-10',
      label: '10 ventes ce mois',
      description: 'Atteignez 10 ventes pour débloquer le statut « Vendeur du mois ».',
      target: 10,
      current: monthSales,
      unit: 'ventes',
    ...
    {
      id: 'revenue-50k',
      label: '50 000 FCFA de revenus',
      description: 'Générez 50 000 FCFA de revenus ce mois-ci.',
      target: 50000,
      current: monthRevenue,
      unit: 'FCFA',
      reward: '💎 Statut « Performer »',
      icon: Star,
    },
    {
      id: 'members-5',
      label: '5 nouveaux membres',
      description: 'Attirez 5 nouveaux membres dans votre communauté.',
      target: 5,
      current: monthMembers,
      unit: 'membres',
      reward: '🌟 Badge Communauté active',
      icon: Flame,
    },
    {
      id: 'content-4',
      label: '4 contenus publiés',
      description: 'Publiez au moins 4 contenus (vidéos, audios, reels).',
      target: 4,
      current: monthContent,
      unit: 'contenus',
      reward: '📹 Badge Créateur prolifique',
      icon: Trophy,
    },
  ], [monthSales, monthRevenue, monthMembers, monthContent]);
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-sm">Défis du mois</h3>
            <p className="text-[10px] text-muted-foreground capitalize">{getMonthLabel()} — {daysLeft}j restants</p>
          </div>
        </div>
            ...
            <div key={challenge.id} className={cn('p-3 rounded-xl border transition-colors', done ? 'border-primary/20 bg-primary/5' : 'border-border')}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] text-muted-foreground">
                  {challenge.id === 'revenue-50k'
                    ? `${(challenge.current / 1000).toFixed(0)}k / ${(challenge.target / 1000).toFixed(0)}k`
      ...
      {completedCount === challenges.length && (
        ...
        >
          <p className="text-xs font-bold text-primary">🏆 Tous les défis accomplis ! Vous êtes un champion !</p>
        </motion.div>
      )}
    </motion.div>
  );
