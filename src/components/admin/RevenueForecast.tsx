...
export function RevenueForecast() {
  ...
  const forecast = useMemo(() => {
    ...
    // Trend: weighted average favoring recent
    ...
    const currentMonthActual = metrics
      .filter((m: any) => new Date(m.metric_date).getMonth() === now.getMonth())
    ...
    const chartData = last30.map((m: any) => ({
      date: new Date(m.metric_date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' }),
      actual: m.revenue || 0,
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h2 className="font-semibold text-sm">Prévision de revenus</h2>
          <p className="text-[10px] text-muted-foreground">Basée sur les 30 derniers jours</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="p-3 rounded-xl bg-primary/5 border border-primary/10 text-center">
          ...
          <p className="text-sm font-bold">{formatCurrency(forecast.projectedTotal, currency)}</p>
          <p className="text-[9px] text-muted-foreground">Projection mois</p>
        </div>
        <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-center">
          <Zap className="h-3.5 w-3.5 text-emerald-400 mx-auto mb-1" />
          <p className="text-sm font-bold">{formatCurrency(forecast.dailyRate, currency)}</p>
          <p className="text-[9px] text-muted-foreground">Rythme / jour</p>
        </div>
        <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/10 text-center">
          ...
          <p className="text-sm font-bold">{forecast.daysLeft}j</p>
          <p className="text-[9px] text-muted-foreground">Restants</p>
        </div>
      </div>
      ...
      <div className={`flex items-center gap-2 p-2 rounded-lg mb-4 text-xs ${
        ...
        <span>
          {forecast.trend === 'up' ? '📈 Tendance haussière' :
           forecast.trend === 'down' ? '📉 Tendance baissière' :
           '➡️ Tendance stable'}
          {' '}({forecast.trendPercent > 0 ? '+' : ''}{forecast.trendPercent}% vs moyenne 30j)
        </span>
      </div>
      ...
      <div className="flex items-center gap-4 mt-2 justify-center">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-muted-foreground">Prévision</span>
        </div>
      </div>
    </motion.div>
  );
