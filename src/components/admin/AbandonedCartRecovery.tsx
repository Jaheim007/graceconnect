...
export function AbandonedCartRecovery() {
  ...
  const sendReminder = useMutation({
    mutationFn: async (cartId: string) => {
      ...
      if (!cart?.email) throw new Error(isFr ? "Pas d'email" : 'No email');
    ...
    onSuccess: () => {
      toast.success(isFr ? 'Rappel envoyé !' : 'Reminder sent!');
      qc.invalidateQueries({ queryKey: ['abandoned-carts'] });
    },
    onError: () => toast.error(isFr ? "Erreur lors de l'envoi" : 'Error sending reminder'),
  });
  ...
  if (isLoading || carts.length === 0) return null;
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="font-semibold text-sm">{isFr ? 'Paniers abandonnés' : 'Abandoned carts'}</h2>
            <p className="text-[10px] text-muted-foreground">
              {activeCarts.length} {isFr ? 'actifs' : 'active'} · {recoveryRate}% {isFr ? 'récupérés' : 'recovered'}
            </p>
          </div>
        </div>
        <Badge variant="outline" className="text-[10px]">{carts.length} total</Badge>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/10 text-center">
          ...
          <p className="text-[10px] text-muted-foreground">{isFr ? 'En attente' : 'Pending'}</p>
        </div>
        <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-center">
          <p className="text-lg font-bold text-emerald-400">{convertedCarts.length}</p>
          <p className="text-[10px] text-muted-foreground">{isFr ? 'Récupérés' : 'Recovered'}</p>
        </div>
        <div className="p-3 rounded-xl bg-primary/5 border border-primary/10 text-center">
          <p className="text-lg font-bold text-primary">{recoveryRate}%</p>
          <p className="text-[10px] text-muted-foreground">{isFr ? 'Taux récup.' : 'Recovery rate'}</p>
        </div>
      </div>
        ...
        {activeCarts.slice(0, 10).map(cart => (
          <div key={cart.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/30 border border-border/50">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium truncate">{cart.product?.title || (isFr ? 'Produit' : 'Product')}</p>
              <p className="text-[10px] text-muted-foreground">
                {cart.buyer_name || cart.email || (isFr ? 'Visiteur' : 'Visitor')} · {formatDistanceToNow(new Date(cart.opened_at), { addSuffix: true, locale: dateFnsLocale })}
              </p>
              {cart.reminder_sent_count > 0 && (
                <div className="flex items-center gap-1 mt-0.5">
                  <Clock className="h-2.5 w-2.5 text-muted-foreground" />
                  <span className="text-[9px] text-muted-foreground">
                    {cart.reminder_sent_count} {isFr ? 'rappel' : 'reminder'}{cart.reminder_sent_count > 1 ? 's' : ''}
                  </span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              {cart.product?.price && (
                <span className="text-xs font-semibold text-primary">
                  {fmt(cart.product.price, cart.product.currency)}
                </span>
              )}
              {cart.email && cart.reminder_sent_count < 3 && (
                <Button size="sm" variant="ghost" className="h-7 w-7 p-0"
                  onClick={() => sendReminder.mutate(cart.id)} disabled={sendReminder.isPending}
                >
      ...
      {activeCarts.length === 0 && (
        <div className="text-center py-6">
          ...
          <p className="text-xs text-muted-foreground">
            {isFr ? 'Aucun panier abandonné actif 🎉' : 'No active abandoned carts 🎉'}
          </p>
        </div>
      )}
    </motion.div>
