    ...
    try {
      ...
      const actionInstruction: Record<'regenerate' | 'amplify' | 'custom', string> = {
        ...
        amplify: isEnglishBook
          ? `Deepen and EXPAND the chapter "${currentChapter.title}" significantly. You MUST return the COMPLETE amplified chapter from start to finish — never stop mid-sentence or mid-paragraph. Add more substance, more concrete arguments, more analysis, and more depth to each point. Make it at least 50% longer and richer than the original.${hasStyleRef ? ` The amplified content MUST stay in the style of "${state.styleReference!.trim()}" — use their vocabulary, their rhetorical patterns, their way of making arguments.` : ''} Do NOT change the subject or add topics that weren't in the original. Just go DEEPER into what's already there. CRITICAL: Do NOT invent fictional characters, fictional personal stories, or fictional anecdotes (like "Sarah", "John", etc.). Only use real facts, statistics, studies, or general scenarios. If the original content already contains specific stories or characters, you may expand on those — but NEVER create new fictional ones. The output must be a COMPLETE, FINISHED chapter.`
          : `Enrichis et DÉVELOPPE significativement le chapitre "${currentChapter.title}". Tu DOIS retourner le chapitre amplifié COMPLET du début à la fin — ne t'arrête JAMAIS en pleine phrase ou en plein paragraphe. Ajoute plus de substance, plus d'arguments concrets, plus d'analyse, et plus de profondeur à chaque point. Rends-le au moins 50% plus long et plus riche que l'original.${hasStyleRef ? ` Le contenu amplifié DOIT rester dans le style de « ${state.styleReference!.trim()} » — utilise son vocabulaire, ses schémas rhétoriques, sa façon d'argumenter.` : ''} Ne change PAS le sujet et n'ajoute pas de thèmes absents de l'original. Va juste PLUS EN PROFONDEUR dans ce qui est déjà là. CRITIQUE : N'invente PAS de personnages fictifs, d'histoires personnelles fictives ou d'anecdotes fictives (comme « Sarah », « Jean », etc.). Utilise uniquement des faits réels, des statistiques, des études ou des scénarios généraux. Si le contenu original contient déjà des histoires ou personnages spécifiques, tu peux les développer — mais ne crée JAMAIS de nouveaux personnages fictifs. Le résultat doit être un chapitre COMPLET et TERMINÉ.`,
