...
export function ShareableEarningsCard() {
  ...
  const copyStats = () => {
    const text = isFr
      ...
      : `🏆 My Siteviral Earnings\n💰 ${fmtEarned} earned\n📊 ${stats.totalSales} sales\nJoin me → siteviral.com`;
  ...
  return (
    <motion.div ref={cardRef} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-bold text-sm">{isFr ? 'Mes gains ambassadeur' : 'My ambassador earnings'}</h3>
      </div>
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="text-center p-3 rounded-xl bg-background/60 border border-border/50">
          ...
          <p className="text-lg font-black text-emerald-600 dark:text-emerald-400">{fmtEarned}</p>
          <p className="text-[9px] text-muted-foreground">{isFr ? 'Gagnés' : 'Earned'}</p>
        </div>
        <div className="text-center p-3 rounded-xl bg-background/60 border border-border/50">
          <Share2 className="h-4 w-4 text-primary mx-auto mb-1" />
          <p className="text-lg font-black">{stats.totalSales}</p>
          <p className="text-[9px] text-muted-foreground">{isFr ? 'Ventes' : 'Sales'}</p>
        </div>
        <div className="text-center p-3 rounded-xl bg-background/60 border border-border/50">
          <Users className="h-4 w-4 text-amber-500 mx-auto mb-1" />
          <p className="text-lg font-black">{stats.linkCount}</p>
          <p className="text-[9px] text-muted-foreground">{isFr ? 'Liens actifs' : 'Active links'}</p>
        </div>
      </div>
      <div className="flex gap-2">
        <Button size="sm" className="flex-1 gap-2 bg-emerald-600 hover:bg-emerald-700 text-white" onClick={shareOnWhatsApp}>
          <Share2 className="h-3.5 w-3.5" /> {isFr ? 'Partager mes gains' : 'Share my earnings'}
        </Button>
      ...
      <p className="text-[9px] text-muted-foreground text-center mt-3">
        🏅 {displayName} • {isFr ? 'Ambassadeur' : 'Ambassador'} Siteviral
      </p>
    </motion.div>
  );
}
