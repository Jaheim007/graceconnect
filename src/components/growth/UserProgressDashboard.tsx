...
/**
 * UserProgressDashboard — unified stats widget showing all user activity
 * Uses progressive disclosure: only shows sections with data
 */
export function UserProgressDashboard({ className }: UserProgressDashboardProps) {
  const { user } = useAuth();
  const { userOrgs } = useOrg();
  const currency = userOrgs[0]?.currency || DEFAULT_CURRENCY;
  ...
  const sections = [
    stats.publishedProducts > 0 && {
      ...
      sub: `${stats.totalProductSales} vente${stats.totalProductSales !== 1 ? 's' : ''}`,
    ...
    stats.affiliateLinkCount > 0 && {
      ...
      sub: `${stats.totalClicks} clic${stats.totalClicks !== 1 ? 's' : ''}`,
  ...
  return (
    <div className={cn('space-y-4', className)}>
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold">Ma progression</h3>
      </div>

      <div className={cn('grid gap-3', sections.length <= 2 ? 'grid-cols-2' : 'grid-cols-2')}>
        {sections.map((s, i) => {
          const Icon = s.icon;
          return (
            <motion.div
