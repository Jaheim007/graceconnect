...
import { eventsExtraCharge } from "@/lib/verticals/extraCharges.functions";
...
export default function EventsExtraCharges({ bookingId, currency, isClient, isProvider, bookingStarted, bookingCompleted }: Props) {
  ...
  const { data: charges } = useQuery({
    queryKey: ["events-extras", bookingId],
    queryFn: async () => {
      const { data } = await supabase.from("events_extra_charges")
  ...
  const create = async () => {
    setSaving(true);
    const res: any = await runExtraCharge({ data: { action: "create", booking_id: bookingId, amount: Number(amount), description: description.trim(), return_origin: window.location.origin } }).catch((e: any) => ({ error: e?.message || "Request failed" }));
    ...
    qc.invalidateQueries({ queryKey: ["events-extras", bookingId] });
            ...
            <DialogContent className="max-w-sm">
              ...
              <div className="space-y-3">
                <Textarea rows={2} value={description} onChange={(e) => setDescription(e.target.value)}
                  placeholder={t("Motif (ex : heure supplémentaire)", "Reason (e.g. extra hour)")} />
                ...
                <Button onClick={create} disabled={saving || !amount || description.trim().length < 3}
                  className="w-full bg-gradient-to-br from-fuchsia-500 to-violet-500 text-white">
