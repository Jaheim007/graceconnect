...
function generateNotification(isFr: boolean): Notification {
  ...
  const templates: (() => Notification)[] = isFr ? [
    ...
    () => ({ id, icon: <Zap className="h-4 w-4 text-purple-500" />, text: `${name} recommande « ${prod} »`, subtext: `${cityLabel} · ${time}` }),
    ...
    () => ({ id, icon: <Zap className="h-4 w-4 text-purple-500" />, text: `${name} recommends "${prod}"`, subtext: `${cityLabel} · ${time}` }),
