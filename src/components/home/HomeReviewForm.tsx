...
export default function HomeReviewForm({ bookingId, providerId, onSubmitted }: Props) {
  const { user } = useAuth();
  ...
  const submit = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from("home_reviews").insert({
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <Textarea rows={3} value={comment} onChange={(e) => setComment(e.target.value)}
        placeholder={t("Comment s'est passée la prestation ?", "How did the job go?")} />
      <Button onClick={submit} disabled={saving || rating < 1} className="w-full mt-3 bg-gradient-to-br from-sky-500 to-emerald-500 text-white">
