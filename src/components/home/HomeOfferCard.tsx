...
import { homeCreateBooking } from "@/lib/verticals/bookings.functions";
...
export default function HomeOfferCard({ offerId }: Props) {
  ...
  useEffect(() => {
    supabase.from("home_offers").select("*").eq("id", offerId).maybeSingle()
  ...
  const runCreateBooking = useServerFn(homeCreateBooking);
