...
interface Props { extraChargeId: string; }

export default function EducationExtraChargeCard({ extraChargeId }: Props) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    supabase.from("education_extra_charges").select("*").eq("id", extraChargeId).maybeSingle()
      .then(({ data }) => setEc(data));
  }, [extraChargeId]);

  if (!ec) return null;
  const isStudent = user?.id === ec.student_id;
  ...
  return (
    <div className="rounded-2xl border-2 border-amber-500/30 bg-amber-50/50 dark:bg-amber-950/20 p-3 text-sm">
      <div className="text-[10px] font-bold uppercase tracking-wider text-amber-600">{t("Supplément", "Extra charge")}</div>
      <div className="mt-1 font-semibold">{ec.label}</div>
      {ec.reason && <div className="text-xs text-muted-foreground">{ec.reason}</div>}
      <div className="mt-2 flex items-center justify-between">
        <span className="text-lg font-black text-amber-600">+{Number(ec.amount_xof).toLocaleString()} XOF</span>
        {ec.status === "pending" && isStudent && (
        ...
        {ec.status === "rejected" && <span className="text-[10px] font-bold uppercase text-muted-foreground">{t("Refusé", "Declined")}</span>}
        {ec.status === "pending" && !isStudent && <span className="text-[10px] font-bold uppercase text-amber-600">{t("En attente", "Pending")}</span>}
      </div>
    </div>
  );
}
