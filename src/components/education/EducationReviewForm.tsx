...
export default function EducationReviewForm({ bookingId, tutorId, onSubmitted }: Props) {
  const { user } = useAuth();
  ...
  const submit = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from("education_reviews").insert({
      booking_id: bookingId, student_id: user.id, tutor_id: tutorId,
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      <Textarea rows={3} value={comment} onChange={(e) => setComment(e.target.value)}
        placeholder={t("Comment s'est passée la séance ?", "How was the lesson?")} />
      <Button onClick={submit} disabled={saving || rating < 1} className="w-full mt-3 bg-gradient-to-br from-teal-500 to-cyan-500 text-white">
