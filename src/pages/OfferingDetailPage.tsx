...
export default function OfferingDetailPage() {
  ...
  const fmt = (n: number) =>
    new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 0 }).format(n) + ` ${offering?.currency || 'XOF'}`;
  ...
  if (!offering) {
    return (
      <EmptyState
        ...
        description="Ce don n'existe pas ou a été retiré."
  ...
  return (
    <div className="min-h-screen bg-background" style={orgThemeStyle}>
      <SEOHead
        ...
        description={offering.description?.slice(0, 155) || `Soutenez ${offering.title} sur Siteviral`}
            ...
            <div className="md:hidden space-y-2">
              ...
              {offering.is_recurring_allowed && (
                <Badge variant="secondary" className="text-xs gap-1">
                  <RefreshCw className="h-3 w-3" /> Ponctuel ou récurrent
                </Badge>
              )}
            </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="md:sticky md:top-16 md:self-start space-y-4">
            <div className="p-5 rounded-2xl border border-border bg-card shadow-card space-y-4">
              ...
              <div className="space-y-2">
                <p className="text-xs font-medium text-muted-foreground">Montants suggérés</p>
              ...
              >
                <HandHeart className="h-5 w-5" /> Faire un don
              </Button>
