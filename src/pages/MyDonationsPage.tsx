...
export default function MyDonationsPage() {
  ...
  return (
    <div className="container max-w-2xl px-4 py-5 sm:py-6 space-y-5">
      <SEOHead title="Mes dons — Siteviral" noindex />
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold flex items-center gap-2">
            <Heart className="h-5 w-5 text-rose-500" /> Mes dons
          </h1>
          <p className="text-xs text-muted-foreground">
            {donations.length > 0 ? `${donations.length} don(s) · Total : ${fmt(total)}` : 'Historique de tes dons et contributions'}
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-muted-foreground mb-4">Tu n'as pas encore fait de don</p>
          <Button onClick={() => navigate('/marketplace?tab=campaigns')} className="gap-2">
            <Heart className="h-4 w-4" /> Voir les campagnes actives
          </Button>
        </div>
      ) : (
          ...
          {donations.map((don: any) => (
            <div key={don.id} className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {new Date(don.completed_at || don.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
              </div>
