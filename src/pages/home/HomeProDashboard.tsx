...
import { Calendar, MessageSquare, Wrench, ShieldCheck, TrendingUp, Home as HomeIcon } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-28">
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/90 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center gap-2 px-4">
          ...
          <div className="leading-tight flex-1 min-w-0">
            ...
            <div className="text-[10px] text-muted-foreground">{t("Espace pro Home", "Home pro space")}</div>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-3xl px-4 py-5 space-y-4">
        {!kycDone && (
          <Link to="/home/pro/kyc" className="block rounded-2xl border border-amber-500/40 bg-amber-500/10 p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{t("Travaux terminés", "Jobs done")}</div>
            <div className="mt-1 text-2xl font-black">{stats?.jobs_completed ?? 0}</div>
          </div>
          <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <div className="grid gap-2.5">
          {[
            { icon: MessageSquare, label: t("Messages", "Messages"), to: "/home/messages" },
            { icon: Calendar, label: t("Mes interventions", "My jobs"), to: "/home/bookings" },
            { icon: Wrench, label: t("Mes services", "My services"), to: "/home/pro/services" },
            { icon: TrendingUp, label: t("Revenus & paiements", "Revenue & payouts"), to: "/home/pro/revenue" },
          ].map((a) => (
