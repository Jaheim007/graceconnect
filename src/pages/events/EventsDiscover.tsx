...
import { ArrowLeft, MapPin, Star, PartyPopper } from "lucide-react";
...
export default function EventsDiscover() {
  ...
  const { data: providers, isLoading } = useQuery({
    queryKey: ["events-discover", activeCat],
    queryFn: async () => {
      let query = supabase.from("events_providers")
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <SEOHead title={isFr ? "Trouver un prestataire — SiteViral Events" : "Find a vendor — SiteViral Events"}
        description={isFr ? "Photographes, DJ, traiteurs, décoration, salles près de toi." : "Photographers, DJs, caterers, decorators, venues near you."} />
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/90 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center gap-2 px-4">
          <Link to="/events" className="rounded-lg p-2 hover:bg-accent"><ArrowLeft className="h-4 w-4" /></Link>
          <h1 className="text-sm font-bold">{isFr ? "Trouver un prestataire" : "Find a vendor"}</h1>
        </div>
      </header>
        ...
        <div className="-mx-4 overflow-x-auto px-4">
          <div className="flex gap-2">
            <button onClick={() => setParams({})} className={cn(
              ...
              !activeCat ? "border-fuchsia-500 bg-fuchsia-500/10 text-fuchsia-600" : "border-border bg-card",
            ...
            {EVENTS_CATEGORIES.map((c) => (
              <button key={c.id} onClick={() => setParams({ cat: c.id })}
                className={cn("shrink-0 inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold",
                  activeCat === c.id ? "border-fuchsia-500 bg-fuchsia-500/10 text-fuchsia-600" : "border-border bg-card")}>
        ...
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center">
            ...
            <p className="mt-3 text-sm font-semibold">
              {isFr ? "Aucun prestataire vérifié pour le moment" : "No verified vendors yet"}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              {isFr ? "Reviens bientôt — nous accueillons de nouveaux prestataires chaque jour."
                    : "Come back soon — we onboard new vendors daily."}
            </p>
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2">
            {filtered.map((p: any) => (
              <Link key={p.id} to={`/events/pro/${p.slug}`}
                ...
                {p.avatar_url ? (
                  ...
                  <div className="grid h-14 w-14 place-items-center rounded-xl bg-fuchsia-100 text-fuchsia-600 dark:bg-fuchsia-500/15 dark:text-fuchsia-400 font-bold">
                    {p.business_name?.[0] ?? "?"}
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-[11px]">
                    ...
                    {p.is_new && <span className="rounded-full bg-violet-500/15 px-1.5 py-0.5 text-[9px] font-bold text-violet-600">{isFr ? "Nouveau" : "New"}</span>}
                  </div>
