    ...
    <div className="pb-24 lg:pb-8">
      <header className="lg:hidden sticky top-0 z-20 border-b border-border/60 bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center gap-2 px-4">
          <Link to="/events/pro" className="rounded-lg p-2 hover:bg-accent"><ClipboardList className="h-4 w-4" /></Link>
      ...
      <div className="mx-auto max-w-4xl px-4 lg:px-8 py-6 space-y-4">
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(orders ?? []).map((o: any) => (
              <Link key={o.id} to={`/events/booking/${o.id}`} className="block rounded-2xl border border-border bg-card p-4 hover:shadow-sm transition">
