...
export default function EventsProRevenuePane() {
  const { user } = useAuth();
  ...
  const { data: provider } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const { data } = await supabase.from("events_providers").select("id, currency, kyc_verified_at").eq("user_id", user!.id).maybeSingle();
      return data;
    },
  });
  ...
  const { data: completed } = useQuery({
    queryKey: ["events-completed", (provider as any)?.id],
    enabled: !!(provider as any)?.id,
    queryFn: async () => {
      const { data } = await supabase.from("events_bookings")
        .select("id, price, currency, event_date, created_at")
        .eq("provider_id", (provider as any).id).eq("status", "completed")
        ...
        {!kycDone && (
          <div className="rounded-2xl border border-amber-500/40 bg-amber-500/10 p-4">
            <div className="flex items-center gap-3">
              ...
              <Button asChild size="sm"><Link to="/admin/events-service/kyc">{t("Faire le KYC", "Verify")}</Link></Button>
            </div>
          </div>
        )}

        <div className="rounded-2xl border border-border bg-card">
          <div className="border-b border-border px-4 py-3 text-sm font-bold">{t("Événements terminés", "Completed events")}</div>
          <div className="divide-y divide-border">
            {(completed ?? []).length === 0 ? (
              <div className="p-6 text-center text-xs text-muted-foreground">{t("Pas encore d'événement terminé.", "No completed events yet.")}</div>
            ) : (completed ?? []).map((b: any) => (
              <div key={b.id} className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="text-sm font-semibold">
                  {b.event_date ? new Date(b.event_date).toLocaleDateString(isFr ? "fr-FR" : "en-US") : new Date(b.created_at).toLocaleDateString(isFr ? "fr-FR" : "en-US")}
                </div>
                <div className="text-sm font-black text-emerald-600">+{Number(b.price).toLocaleString()} {b.currency}</div>
              </div>
            ))}
          </div>
        </div>
