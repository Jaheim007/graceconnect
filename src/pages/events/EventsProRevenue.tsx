...
export default function EventsProRevenue() {
  const { user } = useAuth();
  ...
  const { data: recent } = useQuery({
    queryKey: ["events-recent-bookings", provider?.id],
    enabled: !!provider?.id,
    queryFn: async () => {
      const { data } = await supabase.from("events_bookings")
        .select("id, status, price, currency, event_date, created_at")
      ...
      <div className="mx-auto max-w-3xl px-4 py-5 space-y-4">
            ...
            {(recent ?? []).map((b: any) => (
              <div key={b.id} className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  <div className="text-sm font-semibold">{b.event_date ? new Date(b.event_date).toLocaleDateString(isFr ? "fr-FR" : "en-US") : new Date(b.created_at).toLocaleDateString(isFr ? "fr-FR" : "en-US")}</div>
        ...
        <div className="rounded-2xl border border-dashed border-border bg-card p-5 text-center">
          ...
          <Button asChild variant="outline" size="sm" className="mt-3">
            <Link to="/events/pro/kyc">{t("Voir le KYC", "View KYC")}</Link>
          </Button>
        </div>
      </div>
    </div>
