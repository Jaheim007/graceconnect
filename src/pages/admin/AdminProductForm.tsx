    ...
    <AdminPageShell title={isEdit ? (isFr ? 'Modifier le produit' : 'Edit product') : (isFr ? 'Nouveau produit' : 'New product')} backRoute="/admin/products">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-xl">
        ...
        <div className="flex flex-wrap items-center gap-6">
          <div className="flex items-center gap-2"><Switch checked={watch('is_free')} disabled={watch('is_pwyw')} onCheckedChange={v => { const isAi = !!(studioState || item?.ai_generated); if (v && isAi) { toast({ title: isFr ? 'Non autorisé' : 'Not allowed', description: isFr ? 'Les produits générés par IA ne peuvent pas être gratuits.' : 'AI-generated products cannot be free.', variant: 'destructive' }); return; } setValue('is_free', v); }} /><Label className={cn('text-sm cursor-pointer', watch('is_pwyw') && 'opacity-50')}>{isFr ? 'Gratuit' : 'Free'}</Label>{watch('is_pwyw') && <span className="text-[10px] text-muted-foreground">{isFr ? '(désactivé avec PWYW)' : '(disabled with PWYW)'}</span>}{(studioState || item?.ai_generated) && <span className="text-[10px] text-amber-500 ml-1">🤖 AI</span>}</div>
