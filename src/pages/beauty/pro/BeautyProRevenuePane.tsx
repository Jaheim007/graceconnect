...
export default function BeautyProRevenuePane() {
  const { user } = useAuth();
  ...
  const { data: stats } = useQuery({
    queryKey: ["beauty-provider-stats", provider?.id],
    enabled: !!provider?.id,
    queryFn: async () => {
      const { data } = await supabase.from("beauty_provider_stats").select("*").eq("provider_id", provider!.id).maybeSingle();
      return data;
    },
  });

  const { data: completed } = useQuery({
    queryKey: ["beauty-completed", provider?.id],
    enabled: !!provider?.id,
    queryFn: async () => {
      const { data } = await supabase.from("beauty_bookings")
        .select("id, price_amount, price_xof, currency, slot_start, created_at")
      ...
      <div className="mx-auto max-w-4xl px-4 lg:px-8 py-6 space-y-4">
        <div className="hidden lg:block">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {t("Suis ce que tu as gagné et gère tes retraits.", "Track what you've earned and manage payouts.")}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-2xl border border-border bg-card p-4">
            ...
            <div className="mt-1 text-2xl font-black">
              {Number((stats as any)?.revenue_30d ?? 0).toLocaleString()} <span className="text-xs text-muted-foreground">{currency}</span>
            </div>
          </div>
          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
            ...
            <div className="mt-1 text-2xl font-black">
              {Number((stats as any)?.revenue_all_time ?? 0).toLocaleString()} <span className="text-xs text-muted-foreground">{currency}</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card">
          <div className="border-b border-border px-4 py-3 text-sm font-bold">{t("Prestations terminées", "Completed appointments")}</div>
          <div className="divide-y divide-border">
            {(completed ?? []).length === 0 ? (
              <div className="p-6 text-center text-xs text-muted-foreground">{t("Pas encore de prestation terminée.", "No completed appointments yet.")}</div>
            ) : (completed ?? []).map((b: any) => {
              const price = b.price_amount ?? b.price_xof;
              return (
                <div key={b.id} className="flex items-center justify-between gap-3 px-4 py-3">
                  <div className="text-sm font-semibold">
                    {b.slot_start ? new Date(b.slot_start).toLocaleDateString(isFr ? "fr-FR" : "en-US") : new Date(b.created_at).toLocaleDateString(isFr ? "fr-FR" : "en-US")}
                  </div>
                  <div className="text-sm font-black text-emerald-600">+{Number(price ?? 0).toLocaleString()} {b.currency ?? "XOF"}</div>
                </div>
              );
            })}
          </div>
