...
export default function BeautyProMessagesPane() {
  const { user } = useAuth();
  ...
  const { data: convs, refetch } = useQuery({
    queryKey: ["beauty-pro-conversations", provider?.id],
    enabled: !!provider?.id,
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel(`beauty-pro-conv-${provider.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "beauty_conversations", filter: `provider_id=eq.${provider.id}` }, () => refetch())
      .subscribe();
    ...
    <div className="flex h-[calc(100dvh-3.5rem)] flex-col lg:flex-row">
      <aside className={cn("flex flex-col border-b lg:border-b-0 lg:border-r border-border/60 bg-background",
        ...
        <header className="flex items-center gap-2 px-4 py-3 border-b border-border/60">
          <Link to="/beauty/pro" className="lg:hidden rounded-lg p-1.5 hover:bg-accent"><ArrowLeft className="h-4 w-4" /></Link>
        ...
        <div className="flex-1 overflow-y-auto">
          {(convs ?? []).length === 0 ? (
            ...
            <ul className="divide-y divide-border/60">
              {(convs ?? []).map((c: any) => {
                const name = c.client?.display_name || (isFr ? "Cliente" : "Client");
                const initial = (name[0] ?? "?").toUpperCase();
                const isActive = c.id === activeId;
                return (
                  <li key={c.id}>
                    <NavLink to={`/beauty/pro/messages/${c.id}`}
                      ...
                      {c.client?.avatar_url ? (
                        ...
                        <div className="grid h-10 w-10 place-items-center rounded-full bg-pink-100 text-pink-700 font-bold dark:bg-pink-500/15 dark:text-pink-300">{initial}</div>
                      )}
