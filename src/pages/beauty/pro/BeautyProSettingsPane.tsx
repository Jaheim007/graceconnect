...
import { ArrowLeft, ShieldCheck, Zap, Share2, User, Bell, Copy, ExternalLink } from "lucide-react";
...
export default function BeautyProSettingsPane() {
  const { user } = useAuth();
    ...
    <div className="pb-24 lg:pb-8">
      ...
      <div className="mx-auto max-w-3xl px-4 lg:px-8 py-6 space-y-6">
        <div className="hidden lg:block">
          <h1 className="text-2xl font-black">{t("Réglages de mon salon", "My salon settings")}</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {t("Gère ce qui est visible pour tes clientes et ce qui déclenche les paiements.", "Manage what clients see and what unlocks payouts.")}
          </p>
        </div>

        <section className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-xl bg-pink-500/15 text-pink-600"><Share2 className="h-4 w-4" /></div>
            <div>
              <div className="text-sm font-bold">{t("Ma page publique", "My public page")}</div>
              <div className="text-xs text-muted-foreground">{t("Le lien à partager pour être trouvée & réservée.", "The link to share so clients can find & book you.")}</div>
            </div>
          </div>
          {publicUrl ? (
        ...
        <Row icon={<Scissors className="h-4 w-4" />} iconBg="bg-pink-500/15 text-pink-600"
        ...
        <Row icon={<Bell className="h-4 w-4" />} iconBg="bg-fuchsia-500/15 text-fuchsia-600"
          title={t("Notifications", "Notifications")}
          desc={t("Choisis comment tu es avertie des nouvelles demandes.", "Pick how you're notified of new requests.")}
