...
export default function BeautyProOrdersPane() {
  const { user } = useAuth();
  ...
  const { data: orders, isLoading } = useQuery({
    queryKey: ["beauty-pro-orders", provider?.id, status],
    enabled: !!provider?.id,
    queryFn: async () => {
    ...
    <div className="pb-24 lg:pb-8">
      ...
      <div className="mx-auto max-w-4xl px-4 lg:px-8 py-6 space-y-4">
        <div className="hidden lg:block">
          <h1 className="text-2xl font-black">{t("Rendez-vous", "Appointments")}</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {t("Les demandes à confirmer et prestations à réaliser.", "Requests to confirm and services to deliver.")}
          </p>
        </div>
                ...
                <Link key={o.id} to={`/beauty/bookings/${o.id}`} className="block rounded-2xl border border-border bg-card p-4 hover:shadow-sm transition">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1 space-y-1">
                      ...
                      {o.beauty_services?.title && <div className="text-sm font-semibold truncate">{o.beauty_services.title}</div>}
                      {o.slot_start && (
                        <div className="flex items-center gap-1.5 text-xs">
                          <Calendar className="h-3 w-3 text-muted-foreground" />
                          {new Date(o.slot_start).toLocaleString(isFr ? "fr-FR" : "en-US", { dateStyle: "medium", timeStyle: "short" })}
                        </div>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-base font-black text-pink-600">{Number(price ?? 0).toLocaleString()}</div>
                      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{o.currency ?? "XOF"}</div>
                    </div>
                  </div>
                </Link>
              );
