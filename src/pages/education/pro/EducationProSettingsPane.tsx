...
import { ArrowLeft, ShieldCheck, BookOpen, Share2, User, Bell, Copy, ExternalLink } from "lucide-react";
...
export default function EducationProSettingsPane() {
  const { user } = useAuth();
  ...
  const publicUrl = T?.slug ? `${window.location.origin}/learn/pro/${T.slug}` : null;
    ...
    <div className="pb-24 lg:pb-8">
      ...
      <div className="mx-auto max-w-3xl px-4 lg:px-8 py-6 space-y-6">
        <div className="hidden lg:block">
          <h1 className="text-2xl font-black">{t("Réglages de mon activité", "My tutoring settings")}</h1>
          <p className="text-sm text-muted-foreground mt-1">{t("Gère ce qui est visible et ce qui déclenche les paiements.", "Manage what students see and what unlocks payouts.")}</p>
        </div>

        <section className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-xl bg-teal-500/15 text-teal-600"><Share2 className="h-4 w-4" /></div>
            <div>
              <div className="text-sm font-bold">{t("Ma page publique", "My public page")}</div>
              <div className="text-xs text-muted-foreground">{t("Le lien à partager pour être trouvé & réservé.", "The link to share so students can find & book you.")}</div>
            </div>
          </div>
          {publicUrl ? (
        ...
        <Row icon={<ShieldCheck className="h-4 w-4" />} iconBg="bg-amber-500/15 text-amber-600"
          ...
          cta={kycDone ? t("Voir", "View") : t("Faire le KYC", "Verify now")} to="/admin/learn/kyc"
        ...
        <Row icon={<BookOpen className="h-4 w-4" />} iconBg="bg-teal-500/15 text-teal-600"
          title={t("Mes matières & tarifs", "My subjects & pricing")}
          desc={t("Matières, niveaux et tarifs horaires.", "Subjects, levels and hourly rates.")}
          cta={t("Gérer", "Manage")} to="/admin/learn/subjects" />
        ...
        <Row icon={<Bell className="h-4 w-4" />} iconBg="bg-cyan-500/15 text-cyan-600"
