...
export default function EducationProOrdersPane() {
  const { user } = useAuth();
  ...
  const { data: orders, isLoading } = useQuery({
    queryKey: ["education-pro-orders", (tutor as any)?.id, status],
    enabled: !!(tutor as any)?.id,
    queryFn: async () => {
      let q = supabase.from("education_bookings")
        .select("id, status, scheduled_at, subject, total_xof, duration_min, mode, created_at, student_id")
    ...
    <div className="pb-24 lg:pb-8">
      <header className="lg:hidden sticky top-0 z-20 border-b border-border/60 bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center gap-2 px-4">
          <Link to="/learn/pro" className="rounded-lg p-2 hover:bg-accent"><ClipboardList className="h-4 w-4" /></Link>
          <h1 className="text-sm font-bold">{t("Séances", "Sessions")}</h1>
        </div>
      </header>

      <div className="mx-auto max-w-4xl px-4 lg:px-8 py-6 space-y-4">
        <div className="hidden lg:block">
          <h1 className="text-2xl font-black">{t("Séances & réservations", "Sessions & bookings")}</h1>
          <p className="text-sm text-muted-foreground mt-1">{t("Les demandes à confirmer et cours à donner.", "Requests to confirm and lessons to teach.")}</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(orders ?? []).map((o: any) => (
              <Link key={o.id} to={`/learn/booking/${o.id}`} className="block rounded-2xl border border-border bg-card p-4 hover:shadow-sm transition">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1 space-y-1">
                    ...
                    <div className="text-sm font-semibold flex items-center gap-2">
                      <BookOpen className="h-3.5 w-3.5 text-muted-foreground" />{o.subject}
                    </div>
                    {o.scheduled_at && (
                      <div className="flex items-center gap-1.5 text-xs">
                        <Calendar className="h-3 w-3 text-muted-foreground" />
                        {new Date(o.scheduled_at).toLocaleString(isFr ? "fr-FR" : "en-US", { dateStyle: "medium", timeStyle: "short" })}
                        <span className="text-muted-foreground">· {o.duration_min}min · {o.mode}</span>
                      </div>
                    )}
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-base font-black text-teal-600">{Number(o.total_xof).toLocaleString()}</div>
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground">XOF</div>
                  </div>
                </div>
              </Link>
            ))}
