...
export default function EducationMessagesList() {
  const { user } = useAuth();
  const { locale } = useI18n();
  const isFr = locale === "fr";

  const { data: convs, refetch } = useQuery({
    queryKey: ["education-conversations", user?.id],
    enabled: !!user,
    queryFn: async () => {
      const { data } = await supabase.from("education_conversations")
        .select("id, last_message_at, student_id, tutor_id, education_tutors(display_name, avatar_url, user_id)")
  ...
  useEffect(() => {
    if (!user) return;
    const ch = supabase.channel("education-conv-list")
      .on("postgres_changes", { event: "*", schema: "public", table: "education_conversations" }, () => refetch())
      .subscribe();
        ...
        {(convs ?? []).map((c: any) => (
          <Link key={c.id} to={`/education/messages/${c.id}`}
            ...
            {c.education_tutors?.avatar_url ? (
              ...
              <div className="grid h-11 w-11 place-items-center rounded-xl bg-teal-100 text-teal-600 font-bold dark:bg-teal-500/15">
                {(c.education_tutors?.display_name ?? "?")[0]}
              </div>
            )}
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-bold">{c.education_tutors?.display_name}</div>
              <div className="text-[11px] text-muted-foreground">
                {c.last_message_at ? new Date(c.last_message_at).toLocaleString(isFr ? "fr-FR" : "en-US") : "—"}
              </div>
            </div>
          </Link>
        ))}
