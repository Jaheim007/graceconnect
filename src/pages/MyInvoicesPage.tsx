...
export default function MyInvoicesPage() {
  ...
  const handleDownloadInvoice = (item: any, type: 'purchase' | 'donation') => {
    ...
    const orgName = org.name || (isFr ? 'Organisation' : 'Organization');
    const productTitle = type === 'purchase'
      ? (item as any).digital_products?.title || (isFr ? 'Produit' : 'Product')
      : (item as any).donation_campaigns?.title || (isFr ? 'Don' : 'Donation');

    downloadInvoice({
      invoiceNumber: `SV-${item.id.slice(0, 8).toUpperCase()}`,
      date: item.completed_at || item.created_at,
      buyerName: profile?.display_name || user?.email || (isFr ? 'Acheteur' : 'Buyer'),
  ...
  const renderRow = (item: any, type: 'purchase' | 'donation') => {
    const title = type === 'purchase'
      ? (item as any).digital_products?.title || (isFr ? 'Produit' : 'Product')
      : (item as any).donation_campaigns?.title || (isFr ? 'Don libre' : 'Open donation');
    ...
    return (
      <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl border border-border bg-card hover:shadow-card transition-shadow">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">{orgName} — {date.toLocaleDateString(isFr ? 'fr-FR' : 'en-US')}</p>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead title={isFr ? "Mes factures — Siteviral" : "My Invoices — Siteviral"} noindex />
      <div className="sticky top-0 z-10 glass border-b border-border/40 px-4 h-12 flex items-center gap-3">
        ...
        <span className="font-semibold text-sm flex-1">{isFr ? 'Mes factures' : 'My Invoices'}</span>
      </div>

      <div className="container max-w-2xl py-5 space-y-4">
        <p className="text-xs text-muted-foreground">
          {isFr ? "Retrouvez toutes vos factures d'achats et de dons. Téléchargez-les en PDF." : "Find all your purchase and donation invoices. Download them as PDF."}
        </p>
          ...
          <Tabs defaultValue="purchases" className="w-full">
            <TabsList className="w-full">
              <TabsTrigger value="purchases" className="flex-1 gap-1.5">
                <ShoppingBag className="h-3.5 w-3.5" /> {isFr ? 'Achats' : 'Purchases'} ({purchases.length})
              </TabsTrigger>
              <TabsTrigger value="donations" className="flex-1 gap-1.5">
                <Heart className="h-3.5 w-3.5" /> {isFr ? 'Dons' : 'Donations'} ({donations.length})
              </TabsTrigger>
            </TabsList>
            <TabsContent value="purchases" className="space-y-2 mt-3">
              {purchases.length === 0 ? (
                <EmptyState variant="generic" title={isFr ? "Aucun achat" : "No purchases"} description={isFr ? "Vos factures d'achats apparaîtront ici." : "Your purchase invoices will appear here."} />
              ) : purchases.map(p => renderRow(p, 'purchase'))}
            </TabsContent>
            <TabsContent value="donations" className="space-y-2 mt-3">
              {donations.length === 0 ? (
                <EmptyState variant="generic" title={isFr ? "Aucun don" : "No donations"} description={isFr ? "Vos reçus de dons apparaîtront ici." : "Your donation receipts will appear here."} />
