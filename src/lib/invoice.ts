...
export function generateInvoiceHTML(data: InvoiceData): string {
  const loc = data.locale || 'fr';
  const isFr = loc.startsWith('fr');
  const dateLoc = isFr ? 'fr-FR' : 'en-US';
  const formattedDate = new Date(data.date).toLocaleDateString(dateLoc, { day: 'numeric', month: 'long', year: 'numeric' });
  const formattedAmount = `${data.amount.toLocaleString(dateLoc)} ${data.currency}`;
  ...
  const soldByLabel = isFr ? 'Vendu par' : 'Sold by';
  const billedToLabel = isFr ? 'Facturé à' : 'Billed to';
  const descLabel = isFr ? 'Description' : 'Description';
  const refLabel = isFr ? 'Référence' : 'Reference';
  const amountLabel = isFr ? 'Montant' : 'Amount';
  const paymentConfirmed = isFr ? 'Paiement confirmé' : 'Payment confirmed';
  const footerText = isFr
    ? 'Facture générée automatiquement par'
    : 'Invoice automatically generated by';
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <title>${invoiceLabel} ${data.invoiceNumber}</title>
      <style>
      ...
      <div class="page">
        ...
        <div class="parties">
          <div class="party-card seller">
            <div class="party-label">${soldByLabel}</div>
          ...
          <div class="party-card buyer">
            <div class="party-label">${billedToLabel}</div>
        ...
        <div class="table-section">
          <table>
            <thead>
              <tr>
                <th style="width:50%">${descLabel}</th>
                <th>${refLabel}</th>
        ...
        <div class="payment-info">
          <div class="payment-badge">✅ ${paymentConfirmed}${data.paymentMethod ? ` — ${data.paymentMethod}` : ''}</div>
        </div>

        <div class="footer">
          <p>${footerText} <span class="brand">Siteviral</span> — Hacktualiz Inc.</p>
