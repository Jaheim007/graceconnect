...
BEGIN
  ...
  IF _commission_rate IS DISTINCT FROM 20 THEN
    UPDATE public.organizations SET affiliation_commission_percent = _commission_rate WHERE id = _org_id;
  END IF;

  INSERT INTO public.audit_logs (user_id, action, resource_type, resource_id, organization_id, metadata)
  VALUES (_caller, 'studio.book_quick_created', 'digital_product', _product_id, _org_id,
