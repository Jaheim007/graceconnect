...
-- Fix: accept _org_id so the wizard uses the CURRENT platform, not the first one
...
BEGIN
  ...
  -- If a specific org_id was provided, verify the caller is a member with management rights
  ...
  -- Fallback: pick the first org the user owns (legacy behavior)
