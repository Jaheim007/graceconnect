...
-- ============================================
-- FIX: Remove foreign keys to auth.users (use plain UUIDs)
-- ============================================

-- Drop trigger first
DROP TRIGGER IF EXISTS trg_signup_bonus ON public.profiles;
DROP FUNCTION IF EXISTS public.auto_grant_signup_bonus();

-- Drop functions (depend on tables/types)
DROP FUNCTION IF EXISTS public.get_credit_summary(UUID);
DROP FUNCTION IF EXISTS public.grant_cashback_credits(UUID, NUMERIC, TEXT);
DROP FUNCTION IF EXISTS public.complete_credit_purchase(UUID, TEXT);
DROP FUNCTION IF EXISTS public.grant_bonus_credits(UUID, NUMERIC, TEXT, INTEGER);
DROP FUNCTION IF EXISTS public.grant_daily_credits(UUID, NUMERIC);
DROP FUNCTION IF EXISTS public.consume_credits(UUID, NUMERIC, TEXT, TEXT, JSONB);
DROP FUNCTION IF EXISTS public.expire_credit_lots(UUID);
...
DROP TABLE IF EXISTS public.credit_action_pricing;
DROP TABLE IF EXISTS public.credit_packs;

-- Drop types
DROP TYPE IF EXISTS public.credit_tx_type;
DROP TYPE IF EXISTS public.credit_lot_type;
...
-- Recreate tables WITHOUT auth.users FKs
...
('export_pdf', 'Export PDF', 'export', 2.4, 3.8, 'Génération et export PDF professionnel', 15);
