...
BEGIN
  ...
  IF NEW.sale_price IS NOT NULL AND NEW.sale_price > 0 
     AND (NEW.sale_ends_at IS NULL OR NEW.sale_ends_at > now()) THEN
    _effective_price := NEW.sale_price;
  ELSE
    _effective_price := COALESCE(NEW.price, 0);
  END IF;
  
  _potential_earning := _effective_price * _commission / 100;
  ...
  _product_url := '/org/' || COALESCE(_org.slug, _org.id::text) || '/p/' || COALESCE(NEW.slug, NEW.id::text);
