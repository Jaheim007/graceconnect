...
/**
 * Send an email via Resend and log it to email_logs.
...
export async function sendEmail(
  ...
  try {
    ...
    const { template, to, data, organization_id } = options;
    ...
    await db.from('email_logs').insert({
      ...
      metadata: data || {},
    }).then(() => {}).catch((e: Error) => console.error('email_log insert failed:', e));

    return { ok: res.ok, messageId: result.id, error: result.message };
  } catch (err) {
...
export async function sendEmailToOrgAdmins(
  ...
  try {
    ...
    // Get admin emails from auth (via profiles or auth)
    for (const admin of admins) {
      const { data: { user } } = await db.auth.admin.getUserById(admin.user_id);
      if (user?.email) {
        await sendEmail({ template, to: user.email, data, organization_id: organizationId });
      }
    }
  } catch (err) {
