// events-extra-charge: vendor requests an in-event surcharge; client accepts
// (creates a checkout URL) or declines. Actions: create | accept | decline
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      if (!bk.started_at || bk.status !== 'in_progress') return json({ error: 'Event not started' }, 400);

      const { data: prov } = await db.from('events_providers').select('id, user_id').eq('id', bk.provider_id).maybeSingle();
      if (!prov || prov.user_id !== user.id) return json({ error: 'Only the vendor can request' }, 403);

      const { data: charge, error } = await db.from('events_extra_charges').insert({
        booking_id: bk.id,
      ...
      const { data: conv } = await db.from('events_conversations')
        .select('id').eq('provider_id', bk.provider_id).eq('client_id', bk.client_id).maybeSingle();
      if (conv) {
        await db.from('events_messages').insert({
    ...
    if (action === 'decline') {
      ...
      const { data: ec } = await db.from('events_extra_charges').select('*').eq('id', extra_charge_id).maybeSingle();
      ...
      await db.from('events_extra_charges').update({ status: 'declined' }).eq('id', extra_charge_id);
    ...
    if (action === 'accept') {
      ...
      if (ec.expires_at && new Date(ec.expires_at).getTime() < Date.now()) {
        await db.from('events_extra_charges').update({ status: 'declined' }).eq('id', ec.id);
        return json({ error: 'Request expired' }, 410);
      }

      const origin = req.headers.get('origin') || return_origin || 'https://siteviral.com';
      const base = origin.replace(/\/$/, '');
      const successUrl = `${base}/events/booking/${ec.booking_id}?extra=success`;
      const cancelUrl = `${base}/events/booking/${ec.booking_id}?extra=cancelled`;
      ...
      if (AFRICA_MOMO_CURRENCIES.has(currency)) {
        ...
        const gpRes = await fetch('https://geniuspay.ci/api/v1/merchant/payments', {
          ...
          body: JSON.stringify({
            ...
            metadata: { type: 'events_extra_charge', extra_charge_id: ec.id, booking_id: ec.booking_id, client_id: user.id },
          }),
        });
        const gp = await gpRes.json();
        if (!gpRes.ok || !gp?.success || !gp?.data?.checkout_url) {
          console.error('[events-extra-charge] geniuspay fail', gpRes.status, gp);
    ...
    console.error('[events-extra-charge] fatal', e);
