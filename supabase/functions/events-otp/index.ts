// events-otp: dual-OTP flow for Events bookings.
// Actions: generate_start | verify_start | generate_end | verify_end
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'generate_start') {
      ...
      if (!readyForStart) return json({ error: 'Booking not confirmed' }, 400);
    ...
    if (action === 'verify_start') {
      if (!isProvider) return json({ error: 'Only vendor can verify start OTP' }, 403);
      if (!readyForStart) return json({ error: 'Booking not confirmed' }, 400);
    ...
    if (action === 'generate_end') {
      if (!isProvider) return json({ error: 'Only vendor can generate end OTP' }, 403);
      if (bk.status !== 'in_progress') return json({ error: 'Event not in progress' }, 400);
    ...
    if (action === 'verify_end') {
      ...
      const { data: stats } = await db.from('events_provider_stats')
        .select('provider_id, events_completed, revenue_all_time, revenue_30d').eq('provider_id', bk.provider_id).maybeSingle();
      ...
      if (stats) {
        ...
        await db.from('events_provider_stats').insert({
          ...
          events_completed: 1, revenue_all_time: price, revenue_30d: price,
        });
      }
    ...
    console.error('[events-otp] fatal', e);
