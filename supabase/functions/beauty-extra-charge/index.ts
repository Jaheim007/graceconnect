// beauty-extra-charge: provider requests an in-service surcharge; client accepts
// (creates a checkout URL) or declines. Reuses the same Stripe/GeniusPay routing
// as beauty-create-booking so funds land in the same escrow.
//
  ...
  try {
    ...
    if (action === 'create') {
      ...
      if (!booking_id || !amount || !description) {
        return json({ error: 'booking_id, amount, description required' }, 400);
      }
      if (amount < 100) return json({ error: 'Amount too small' }, 400);
      ...
      if (!bk.started_at) return json({ error: 'Service not started yet' }, 400);
      if (!['confirmed'].includes(bk.status)) return json({ error: 'Invalid booking status' }, 400);

      const { data: prov } = await db
        .from('beauty_providers').select('id, user_id')
        .eq('id', bk.provider_id).maybeSingle();
      ...
      const { data: charge, error } = await db
        ...
        .insert({
          ...
          currency: (bk.currency || 'XOF').toUpperCase(),
      ...
      // Post message in the conversation between the parties (if one exists)
      const { data: conv } = await db
        ...
        .maybeSingle();
      if (conv) {
        await db.from('beauty_messages').insert({
    ...
    if (action === 'decline') {
      ...
      const { data: ec } = await db
        .from('beauty_extra_charges').select('*').eq('id', extra_charge_id).maybeSingle();
      ...
      await db.from('beauty_extra_charges').update({ status: 'declined' }).eq('id', extra_charge_id);
    ...
    if (action === 'accept') {
      ...
      if (new Date(ec.expires_at).getTime() < Date.now()) {
        await db.from('beauty_extra_charges').update({ status: 'expired' }).eq('id', ec.id);
        return json({ error: 'Request expired' }, 410);
      }

      const origin = req.headers.get('origin') || body.return_origin || 'https://siteviral.com';
      const successUrl = `${origin.replace(/\/$/, '')}/beauty/bookings/${ec.booking_id}?extra=success`;
      const cancelUrl = `${origin.replace(/\/$/, '')}/beauty/bookings/${ec.booking_id}?extra=cancelled`;
      const currency = String(ec.currency).toUpperCase();
      const amount = ec.amount;
      ...
      if (AFRICA_MOMO_CURRENCIES.has(currency)) {
        ...
        const gpRes = await fetch('https://geniuspay.ci/api/v1/merchant/payments', {
          method: 'POST',
          headers: {
            'X-API-Key': GP_KEY, 'X-API-Secret': GP_SECRET,
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            amount: Math.round(amount),
            currency,
            description: `Supplément — ${ec.description}`.slice(0, 120),
            customer: { email: user.email, name: (user.user_metadata as any)?.full_name },
            success_url: successUrl,
            error_url: cancelUrl,
            metadata: {
              type: 'beauty_extra_charge',
              extra_charge_id: ec.id,
              booking_id: ec.booking_id,
              client_id: user.id,
            },
          }),
        });
        ...
        if (!gpRes.ok || !gp?.success || !gp?.data?.checkout_url) {
          console.error('[beauty-extra-charge] geniuspay fail', gpRes.status, gp);
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{
          ...
          price_data: {
            ...
            product_data: {
              name: `Supplément — ${ec.description}`.slice(0, 120),
            },
          },
        }],
        mode: 'payment',
    ...
    console.error('[beauty-extra-charge] fatal', e);
