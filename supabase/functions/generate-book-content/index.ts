...
REMINDER: ${pages}-page book. Each chapter ≈ ${chapterWordTarget} words. REAL book, not AI text.`;
    ...
    const requestTimeoutMs = singleChapter ? 75_000 : 95_000;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), requestTimeoutMs);
    const maxTokens = singleChapter ? 8000 : (chapterCount >= 8 ? 24000 : 16000);
    ...
    if (!singleChapter && normalizedChapters.length < chapterCount) {
      console.warn(`[generate-book-content] Partial generation: got ${normalizedChapters.length}/${chapterCount} chapters — delivering partial result`);
    }
