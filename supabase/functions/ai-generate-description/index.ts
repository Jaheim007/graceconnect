...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const description = await geminiGenerateText({
      apiKey: GEMINI_API_KEY, model: 'gemini-2.5-flash', prompt, temperature: 0.7, maxOutputTokens: 1000,
    });

    const cleaned = description.replace(/```html\n?/g, '').replace(/```\n?/g, '').trim();
    return jsonResp({ description: cleaned });
  } catch (e: any) {
